...
第一版直接复刻官方 iOS App 的 Codex 相关功能范围，包括：

- Host Profile 管理：支持 Mac/VPS/Linux，多 host 切换。
- SSH 认证：支持 password 和 SSH key，凭据保存到 Keychain，可用 Face ID / 设备解锁保护。
- 轻量远端诊断：检查 SSH、`codex --version`、`app-server proxy`、daemon 启动和 `initialize` 握手。
- workspace / 项目列表：从 `thread/list` 返回的 `Thread.cwd` 聚合 workspace，支持“按项目分组”和“按时间顺序”切换。
- 会话详情：用 `thread/read(includeTurns: true)` 展示历史；继续会话走 `thread/resume -> turn/start`。
- 官方风格输入栏：`+` 菜单、权限盾牌、模型显示、麦克风、发送/停止按钮。
- 附件：相机和文件真实可用，通过 `fs/createDirectory` / `fs/writeFile` 上传远端，再用 `UserInput.localImage` 或远端路径参与 prompt。
- 权限菜单：真实影响 Codex 权限配置，而不是只改 UI。
- 审批流：支持命令执行、文件变更、权限请求的最小可用审批 UI。
- 计划模式：通过官方协议能力对齐 `CollaborationMode` / planning，不用 prompt 前缀模拟。
- 前台恢复和重连：不承诺无后端情况下的可靠后台推送。
